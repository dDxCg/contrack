...
import { RevenueRow, ShiftRepository } from '../Repositories/shift.repository';
import { Page } from '../Repositories/tenant-scoped.repository';
import { StatementListFilter, StatementRepository } from '../Repositories/statement.repository';
import { AccessContext } from './AccessControl/access-context';
import { addMonthsUTC, round2, toDateString } from './period';
...
export class StatementService {
  ...
  /** Blocks while any shift due in the period isn't `completed` — a disputed or evidence-less shift both count (D6). */
  ...
  async export(access: AccessContext, id: number): Promise<{ jobId: string }> {
    const statement = await this.requireStatement(access, id);
    statement.export();
