...
describe('AppModule — storage implementation comes from the environment', () => {
  ...
  it('wires the S3 client behind a resilience wrapper as soon as the bucket variables appear — no code change', async () => {
    ...
    expect(client).toBeInstanceOf(ResilientObjectStorageClient);
  });
});
