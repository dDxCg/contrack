...
export class ContractCostRepository extends TenantScopedRepository<ContractCost> {
  ...
  /** Replaces any existing entry for (contract, category, period) — unique per `04-schema.sql` (FR14). */
  ...
  /** Up to `limit` most recent months with recorded cost, strictly before `beforePeriod` — FR28's trailing average. */
  async monthlyTotalsBefore(
    ...
    )) as { period: Date; total: string }[];

    return rows.map((row) => Number(row.total));
  }

  /** Sum of every category recorded for this exact month, or `null` when nothing was recorded (FR28 falls back). */
  ...
  /** All-time recorded cost tenant-wide — the numerator of FR28's cost-to-revenue ratio. */
