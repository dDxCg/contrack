...
| D9 | **Alert de-duplication is a DB unique constraint, not application state** | US-12/US-13 require "never fires twice" for a given contract/shift. Alternative: an in-memory or job-level dedupe table the alert job consults before sending — rejected, it has to be reinvented correctly by every job runner and every retry path. | `alerts` has `UNIQUE (tenant_id, kind_id, subject_id)` (`04-schema.sql`); the alert job's insert is the dedupe check — a second attempt for the same contract/shift fails the constraint rather than sending twice. `subject_id` has no FK (polymorphic: a contract id or a shift id depending on `kind_id`), mirroring `05-api.yaml`'s `Alert` schema. | A kind needs to re-fire on a state change (e.g. re-alert if a shift stays overdue past a second threshold) — the unique constraint would need a period/threshold column added to stay one-row-per-occurrence rather than one-row-ever. |
| D10 | **Alert job runs in-process on `@nestjs/schedule`, fanned out over active tenants, guarded by a Redis distributed lock** | FR25/FR26 need a daily trigger. Options: an external cron hitting an internal endpoint, or an in-process scheduler. In-process was chosen — one less deployable piece at this scale. Two pods must never double-fire the same daily run, so the `@Cron` tick takes a distributed lock first (`services/alerts/distributed-lock.ts`; Redis `SET NX PX` with an owner-checked release). Without `REDIS_LOCK_URL` configured the app falls back to an in-process lock, which is correct only for a single instance. | `AlertJobScheduler.scheduled()` runs daily at 06:00, acquires `alerts:daily`, and calls `AlertJobService.runAll()`, which iterates `TenantRepository.activeIds()` — suspended tenants are never alerted — and continues past a failing tenant. The lock is released even when the fan-out throws. | A second deployment role (worker process) or a need for per-tenant schedules/catch-up runs appears — the fan-out is one loop and moves behind a queue or external scheduler then. |
| D11 | **Per-tenant timezone, resolved only at the "instant → calendar date" boundary; storage and month arithmetic stay UTC** | The product runs in Vietnam (UTC+7); `tenants.timezone` (`04-schema.sql`, default `Asia/Ho_Chi_Minh`) is the source of truth. Alternative: convert everything to local time at the storage layer — rejected, `shifts.scheduled_date` and `statements.period`/`contract_costs.period` are already plain `DATE` columns with no timezone attached, so there is nothing to convert there; the only place a timezone was ever missing was turning a clock instant (`IClock.now()`) into "today" or "this month" for a default/threshold. | `utils/period.ts` gained `toDateStringInZone`/`startOfMonthInZone` (`Intl.DateTimeFormat` with a `timeZone`), used only where a service turns "now" into a calendar boundary: `AlertJobService.run`'s overdue/expiry "today", `DashboardService`'s default period, `ContractProfitabilityService`'s trailing-months anchor. Every other period computation (statement/reconciliation/cost windows) already takes an explicit period argument and needs no timezone — it was never UTC-versus-local, it was already a fixed calendar month. Fixed alongside: `addMonthsUTC` didn't clamp month-end overflow (`2026-01-31` + 1 month became `2026-03-03`, not `2026-02-28`); `statements.period`/`contract_costs.period` now carry `CHECK (period = date_trunc('month', period))` so a mid-month period can't be stored even by a future caller that forgets to normalize. | A screen needs to *display* times in the tenant's zone rather than just bucket dates by it — that's a presentation concern this ADR doesn't cover and would need its own seam. |
| D12 | **Token revocation shares the same Redis connection as the alert-job distributed lock, behind a `RevocationStore` seam** | Logout must invalidate a credential across every process, and the app already opens a Redis connection for D10's distributed lock. Alternative: a second Redis client, or a separate revocation-only service — rejected, one `REDIS_LOCK_URL` connection serving two unrelated key namespaces (`alerts:*` locks, `revoked:*` sets) is simpler to operate than two connections for one optional dependency. | `RevocationStore` (`services/auth/revocation-store.ts`): `RedisRevocationStore` does `SET revoked:<jti> 1 EX <ttl>` / `EXISTS`, TTL matched to the token's own remaining lifetime so eviction is free; `InMemoryRevocationStore` is the same `CLOCK`-driven fallback used when `REDIS_LOCK_URL` is unset (single-instance only, as D10 already notes). `TokenService` depends on the interface, not a concrete store, so `verify*`/`revoke`/`isRevoked` are async but ignorant of which backend is live. | A revocation needs to trigger a side effect (e.g. push a "signed out elsewhere" notice) — a plain key/TTL store can't carry that; would need pub/sub or an event on top. |
| D13 | **Field-link single-use enforced through the same `RevocationStore`, not a `field_token_used_at` column** | A field token must not be replayable after a successful submit. The remediation plan's original suggestion was a DB column; rejected once `RevocationStore` existed (D12) — a column would duplicate the same "mark a jti dead, evict by TTL" logic the revocation store already provides, for one extra write inside the submit transaction and one extra column with no other reader. | `FieldTokenService.markUsed(claims)` calls `RevocationStore.revoke(jti, exp)` after a successful `FieldSubmissionService.submit()`; `verify()` checks `isRevoked` and throws `token.already_used` (distinct from `Shift.complete()`'s pre-existing `shift.already_completed` guard, which still catches reuse via a *different* token for the same shift). | A field token needs to be revocable *before* its natural expiry for a reason other than use (e.g. an admin invalidating an outstanding link) — the current seam already supports that (`revoke()` takes any jti), so likely no revisit needed, just a new call site. |
