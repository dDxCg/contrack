...
| D28 | **Contract sub-resources (`PATCH /contracts/{id}`, site/item CRUD) never touch the shift schedule; `schedule:regenerate` is deferred, not built** | `05-api.yaml` already specified the full set (updateContract, addSite/updateSite/deleteSite, addServiceItem/updateServiceItem/deleteServiceItem, regenerateSchedule) with roles matching the existing coarse `Contracts` grants exactly, so no `role-resolver.ts` change was needed — `Director` already has every operation, `Manager` already has `Create` (for `addSite`/`addServiceItem`) but not `Update`/`Delete` (site/item edits and deletes stay director-only, matching the spec). `regenerateSchedule` was scoped out for this pass: its correct behavior is a per-item date diff against the newly-desired schedule (new dates created unassigned, dropped dates removed unless the shift is completed/disputed, unchanged dates — including their `assignee_id`/`team_id` — left untouched so a dispatcher's manual decision on a still-valid date survives an item edit), which is real algorithm work distinct from the CRUD in this pass and deserves its own TDD cycle. | `ContractService.addSite`/`updateSite`/`deleteSite`/`addItem`/`updateItem`/`deleteItem`/`update` all persist directly with no shift-schedule side effect — a contract's shifts stay exactly as `create()` generated them until a human calls `DispatchService`/reassigns by hand, or `schedule:regenerate` (once built) reconciles them. DB-level cascade (`ON DELETE CASCADE` from `contracts`→`contract_sites`→`contract_items`→`shifts`, already in `04-schema.sql`) handles delete cleanup; no application-level cascade code was written. | `schedule:regenerate` gets implemented — the date-diff algorithm above is the agreed design, not yet built. |
| D29 | **`DashboardSummary`'s trend arrays each pick their own bucket length rather than sharing one; `profit_trend` is always calendar months** | `new_contracts_trend` and `comparison` reuse a single window generator matched to the query's own `bucket_unit` — count/revenue data has no grain constraint, so the trailing buckets are just repeats of the filtered period's own length (month-stepped in month mode, same-length windows otherwise). `profit_trend` can't follow that: `contract_costs.period` is DB-constrained to the first of a calendar month (`04-schema.sql`'s `ck_contract_costs_period_is_month_start`), so a cost figure only ever exists per calendar month regardless of what the dashboard's own query window is. Rather than degrade profit_trend's accuracy in day/week mode, it always uses 6 trailing calendar months ending at the month containing the filtered period, and `comparison`'s margin_pct — which needs a cost figure too — accepts the same per-contract recorded-or-estimated lookup even when its own window isn't month-aligned (falls through to `CostEstimationService.estimate`, which is grain-agnostic). | `shifts_by_site` (sorted by `completed_pct` ascending, null last), `new_contracts_trend`, `renewal_rate_pct`/`cancellation_rate_pct` (trailing 12 months, fixed, same cohort/denominator), `profit_trend`, and `comparison` (the immediately-preceding same-length window, reusing the same generator as the trend arrays with count=2) all added to `DashboardService.get`. `ContractRepository` gained `countSignedBetween`/`expiryCohortStatusCounts`; `ShiftRepository` gained `statsRowsBySite`. `DashboardService` now also depends on `ContractCostRepository`/`CostEstimationService` (already-registered providers from the contract-costs feature, no new DI wiring). | A trend needs true week-grain cost data — would need `contract_costs.period` to drop its month-start constraint, a bigger schema change than this pass took on. |
