...
Five roles use the system — director, manager, accountant, team lead, employee.
The **customer is outside the system boundary**: they sign the contract and the
paper receipt in person and raise complaints by phone, all of which staff record
on their behalf. Two external services are integrated: Zalo ZNS and/or SMS for
reminders (FR14), and VietQR for payment (Could-have, not in MVP).
...
a closed statement — once at the API boundary, and keep the one path with no login
and the worst network the lightest thing in the system.*
...
```mermaid
flowchart LR
    client["Desk & field browsers"] -->|HTTPS| app

    subgraph host["Single host per customer (C4, D1)"]
        app["App process<br/>API + schedule generator + alert job"]
        db[("Database")]
        store[("Object storage")]
    end
...
| R2 | **`contract_items.frequency` has no grammar.** `VARCHAR(100)` free text, yet FR4's generator must parse it to produce shifts. This is a **schema risk**, not only a documentation gap. | High — blocks FR4, the schedule generator, entirely | High | Define a frequency grammar (e.g. `2/month`) or a lookup table with an explicit interval before the generator is designed further |
...
| Term | Definition |
|---|---|
| Contract | The recurring service agreement between LichHD's operator and a customer company; the entity sites, items, shifts and statements derive from |
