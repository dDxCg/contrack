...
| D15 | **Money gets a value object for JS-side summation only, not a full entity-column type** | Summing several money amounts in application code is exposed to floating-point drift; a database-level sum is not, since the database does exact decimal arithmetic itself. A full value-object treatment (every money-bearing field typed as `Money`, converted at every database boundary) was considered and rejected for now — the read side of this system deliberately does not route through the ORM's own object hydration, so a column-level safeguard would protect writes only, while the actual proven risk lives specifically in application-level summation. | Every application-level sum of money values goes through the value object; a database-level sum does not, since it was never at risk. Money amounts read from and written to the database remain plain numbers everywhere else. | A second class of money bug surfaces outside summation (e.g. a rounding disagreement between the application and the database on a single stored amount) — that would justify carrying the value object further, into storage and every boundary, rather than confining it to arithmetic. |
