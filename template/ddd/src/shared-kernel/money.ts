 ...
 *
 * Ported near-verbatim from backend/utils/money.ts. Same Big.js-backed
 * arithmetic, same half-up rounding to 2 decimal places, same `allocate()`
 * remainder-distribution algorithm. The only thing removed is the TypeORM
 * `moneyTransformer` — domain/ and shared-kernel/ have zero framework
 * dependencies, so the TypeORM <-> Money mapping now lives in
 * infrastructure/persistence next to the persistence entity that needs it.
 */
